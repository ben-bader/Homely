...
List<Property> filter(
    ...
    // ✅ Paginated: Get all properties by boost then created date
    @Query("""
        SELECT p FROM Property p
        LEFT JOIN BoostPurchase b ON b.property.id = p.id AND b.status = 'APPROVED' AND b.expiryAt > :now
        ORDER BY CASE WHEN b.id IS NOT NULL THEN 0 ELSE 1 END ASC,
                 ...
                 p.createdAt DESC
    """)
    Page<Property> findAllOrderByBoostThenCreatedAt(@Param("now") Instant now, Pageable pageable);

    // ✅ Paginated: Filter properties
    @Query("""
        SELECT p FROM Property p
        WHERE (:listingType IS NULL OR p.listingType = :listingType)
        AND (:propertyType IS NULL OR p.propertyType = :propertyType)
    ...
    @Query("""
        ...
        WHERE
            ...
            OR CAST(p.price as string) LIKE CONCAT('%', :keyword, '%')
    """)
    Page<Property> globalSearchPaginated(@Param("keyword") String keyword, Pageable pageable);

    // ✅ Paginated: Get seller properties
