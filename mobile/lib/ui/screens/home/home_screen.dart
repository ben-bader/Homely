...
import 'package:homely/ui/screens/tours/tours_screen.dart';
...
final navBadgesProvider = FutureProvider.autoDispose<_NavBadges>((ref) async {
  ...
  final notificationsAsync = await ref.read(
    notificationsProvider(userId).future,
  );
  final unread = notificationsAsync.where((n) => !n.read).toList();
...
class _SellerListingsTabState extends ConsumerState<_SellerListingsTab> {
  ...
  Future<void> _loadUserId() async {
    _userId = await ref.read(authRepositoryProvider).getCurrentUserId();
    if (mounted) setState(() {});
  }
  ...
  void _onSearchChanged(String value) {
    ...
    final allListings = ref
        ...
        .maybeWhen(data: (l) => l, orElse: () => <PropertyEntity>[]);
...
class _SellerPropertyCard extends ConsumerWidget {
  ...
  Future<void> _deleteProperty(BuildContext context, WidgetRef ref) async {
    try {
      debugPrint('🗑️ Deleting PropertyEntity: ${property.id}');
      final repo = ref.read(propertyRepositoryProvider);
      await repo.delete(property.id);
      debugPrint('✅ PropertyEntity deleted successfully: ${property.id}');
          ...
          SnackBar(
            content: Row(
              children: [
                ...
                const SizedBox(width: 10),
                Text(
                  'PropertyEntity deleted successfully',
                  style: GoogleFonts.outfit(
                    color: Colors.white,
                    fontWeight: FontWeight.w600,
                  ),
      ...
      debugPrint('❌ Error deleting PropertyEntity: $e');
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Row(
              children: [
                ...
                const SizedBox(width: 10),
                Text(
                  'Failed to delete PropertyEntity: ${e.toString()}',
                  style: GoogleFonts.outfit(
                    color: Colors.white,
                    fontWeight: FontWeight.w600,
                  ),
...
// ═════════════════════════════════════════════════════════════════════════════
// PropertyEntity CARD — with search highlight
// ═════════════════════════════════════════════════════════════════════════════
...
// NOTIFICATION BUTTON
// ═════════════════════════════════════════════════════════════════════════════
class _NotifBtn extends ConsumerStatefulWidget {
  final VoidCallback onTap;
  const _NotifBtn({required this.onTap});
  @override
  ConsumerState<_NotifBtn> createState() => _NotifBtnState();
}
